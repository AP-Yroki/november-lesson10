class Publisher:
    ...
    def get_info(self):
        print(f'Издательство {self.name}, находится в {self.location}')
...
class BookPublisher(Publisher):
    def __init__(self, name, location, num_authors):
        super(BookPublisher, self).__init__(name, location)
        self.num_authors = num_authors

    def publish(self, message, author):
        print(f'Передаём рукопись "{message}", написанную автором {author} в издательство {self.name} ({self.location})')

class NewspaperPublisher(Publisher):
    def __init__(self, name, location, num_pages):
        super(NewspaperPublisher, self).__init__(name, location)
